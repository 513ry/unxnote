...
init_window()
{
  ...
  if (!get_primary_monitor_geometry(conn, screen->root, &screen_width,
    ...
    fprintf(stderr, "randr failed to get primary monitor geometry\n");
    return false;
  }
...
/** @TODO: Create windows asynchronously */
xcb_window_t
new_window(uint16_t id)
{
  xcb_window_t window = xcb_generate_id(x_conn.conn);
  xcb_cookie = xcb_create_window(
    ...
    (WINDOW_MARGIN * (id + 1)) - UINT32_C(37),
    WINDOW_WIDTH, WINDOW_MIN_HEIGHT, 0,
...
void free_window()
{
  xcb_generic_error_t *error = xcb_request_check(x_conn.conn, xcb_cookie);
  if (error) {
    printf("XCB error: %d\n", error->error_code);
    free(error);
  }
...
set_window_type(xcb_connection_t *conn, xcb_window_t window)
{
  ...
  if (net_wm_window_type == XCB_NONE || net_wm_window_type_name == XCB_NONE) {
    fprintf(stderr, "Failed to retrieve necessary atoms\n");
    return false;
  }
  ...
  if (motif_wm_hints == XCB_NONE) {
    fprintf(stderr, "Failed to retrieve Motif Hints\n");
    return false;
  }
			     ...
			     uint16_t *x, uint16_t *y)
{
  ...
  if (!primary_reply) {
    fprintf(stderr, "Failed to get primary output\n");
    free(res_reply);
    return false;
  }
